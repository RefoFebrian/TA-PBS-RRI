...
export const POST = async (req: NextRequest) => {
  try {
    ...
    if (existingEmail) {
      return getResponse(1, `email ${email} already exists`, 409, null);
    }

    // kondisi jika username sudah terdaftar
    if (existingUsername) {
      return getResponse(1, `username ${username} already exists`, 409, null);
    }
    ...
    return getResponse(0, "Registrasi berhasil", 201, data);
  } catch (error: unknown) {
